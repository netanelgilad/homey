...
// function sendData(device: any = false, hexData: any = false) {
//   if (device === false || hexData === false) {
//     console.log(
//       "Missing params, sendData failed",
//       typeof device,
//       typeof hexData
//     );
//     return;
...
export function emitCommand(device: Device, command: DeviceCommand) {
    const hexDataBuffer = new Buffer(command.data, "hex");
    device.sendData(hexDataBuffer);
}

// export async function runCommand(command, req, res) {
//   try {
...
//         console.log(
//           `[ERROR] The device at ${device.host.address} (${
//             device.host.macAddress
//           }) doesn't support the sending of IR or RF codes.`
//         );
//       } else if (command.data && command.data.includes("5aa5aa555")) {
//         console.log(
//           '[ERROR] This type of hex code (5aa5aa555...) is no longer valid. Use the included "Learn Code" accessory to find new (decrypted) codes.'
//         );
//       } else {
...
//             let send = commands.find(e => {
//               return e.command == find;
//             });
//             if (send) {
